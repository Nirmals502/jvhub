package com.example.deepsingh.jbhub.Multipart_enttity;
    ...
    public static boolean checkPermission(final Context context)
    {
        ...
        if(currentAPIVersion>=android.os.Build.VERSION_CODES.M)
        {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale((Activity) context, Manifest.permission.CALL_PHONE)) {
                ...
                alertBuilder1.setCancelable(true);
                alertBuilder1.setTitle("Permission necessary");
                alertBuilder1.setMessage("CALL_PHONE permission is necessary");
                alertBuilder1.setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
                    public void onClick(DialogInterface dialog, int which) {
                        ActivityCompat.requestPermissions((Activity) context, new String[]{Manifest.permission.CALL_PHONE}, 125);
                    }
                });
                AlertDialog alert = alertBuilder1.create();
                alert.show();
